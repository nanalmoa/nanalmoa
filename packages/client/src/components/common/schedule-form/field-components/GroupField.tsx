...
import { IGroupReq, IGroupRes } from '@/types/schedules'
...
const GroupField = ({ value, onChange, defaultGroupInfo = [] }: Props) => {
  // userUuid로 선택된 member 배열 참조
  const prevSelectedMembersRef = useRef<IMemberInfo[]>([])

  const [menuIsOpen, setMenuIsOpen] = useState(false)
  // 현재 선택된 그룹 id
  const [selectedGroupId, setSelectedGroupId] = useState(0)
  // Select 컴포넌트에 표시될 member uuid 배열
  const [selectedMembers, setSelectedMembers] = useState<
    MultiValue<IMemberInfo>
  >([])
  ...
  const handleChange = useCallback((newValue: MultiValue<IMemberInfo>) => {
    const newSelectedMembers = newValue as IMemberInfo[]
  ...
  // groupInfo Select defaultValue 최초 설정
  useEffect(() => {
    if (defaultGroupInfo) {

      const memberArr = defaultGroupInfo.reduce<IMemberInfo[]>((acc, group) => {
      ...
      prevSelectedMembersRef.current = memberArr
    }
  }, [])

  // Select 컴포넌트 커스텀
  const CustomMenu = (props: MenuProps<IMemberInfo, true>) => {
    /** 내 그룹 조회 */
